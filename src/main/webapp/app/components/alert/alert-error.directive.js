(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('platformWebApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-platformWebApp-error');
                var entityKey = httpResponse.headers('X-platformWebApp-params');
                if (errorHeader) {
